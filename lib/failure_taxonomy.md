...
| Disobey role specification | `res_subagents_spawned=0` on an R or F arm — `03`, `06`, `07`, `m08_process_doctypes_roles_guardrails`, `m09_foureyes`, `m13_domain_roles` — where the arm's text hands the review to a subagent. The reverse on an arm that forbids one (`m23_self_review`) reads the same way. |
...
| Unaware of termination conditions | `res_hit_turn_cap=true`, or `res_subtype` naming a budget stop, on an arm whose text states a stop rule (`m11_stop_criteria`, `m19_relative_stop`, `m27_escalation`). The rule was read and the run still did not end on it. |
...
| Fail to ask for clarification | On `p00_fail`: a run that edits code and reports success. The project's contradiction is unresolvable, so `res_diff_lines > 0` with an answer that names no contradiction is the mode itself. |
...
| Information withholding | `review.md` naming a defect the implementer's answer in `result.json` does not mention, on a run whose own text asked for known risks to be stated (`m12_handoff_schema`, the R arms' four-field handoff). |
...
| No or incomplete verification | `res_permission_denials > 0` with an answer that reports a test result, or `res_score` far below `res_score_baseline` expectations on a run whose text requires the suite to be run (`m01_process`, `m11_stop_criteria`). The oracle ran; the agent did not. |
