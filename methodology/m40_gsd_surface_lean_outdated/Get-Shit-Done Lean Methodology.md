...
## 0. Source analysis (what GSD actually is)

GSD (gsd-build/get-shit-done, now archived → open-gsd/gsd-core) is ~1900 files:

| Layer | Content | Nature |
|---|---|---|
| `commands/gsd/` (67 md) | slash-command entry points | prompt |
| `get-shit-done/workflows/` (107 md) | numbered step scripts per command | prompt |
| `get-shit-done/references/` (63 md) | gates, TDD, anti-patterns, context budget, thinking models | prompt |
| `get-shit-done/templates/` (46) | STATE / ROADMAP / PLAN / SUMMARY / PROJECT / CONTEXT … | prompt |
| `agents/` (33 md) | role contracts: planner, executor, verifier, plan-checker, researchers, auditors | prompt |
| `hooks/` (15 js/sh) | PreToolUse blocking, commit validation, injection scan, context monitor, statusline | **code** |
| `bin/` + `sdk/` (350) | state parsing, phase/milestone lifecycle, locks, dispatch, worktrees | **code** |
| `.github/`, `scripts/`, `tests/` | repo governance of GSD itself | out of scope for a user |

~90% of the file count that a *user* is exposed to is already markdown prompt.
The code layer does three things markdown cannot: **block** an action, **compute**
state deterministically, and **run unattended**.
...
`NN-CONTEXT.md` under **Locked decisions** (binding) and **Claude's discretion**
...
Failures → `plan-phase N --gaps`: a new plan closing only the gaps. Loop ≤3, then escalate.
...
## 5. Session discipline (replaces the hooks)
...
- **Before any Write/Edit to an existing file**: Read it in this session first.
...
## 7. Reproduction score: **7 / 10**

| GSD capability | Reproduced in pure MD | Score |
|---|---|---|
| Phase lifecycle, artifacts, templates | fully — it was already markdown | 10 |
| Role separation, adversarial review | fully; fresh-context subagents suffice | 10 |
| Gate taxonomy + capped revision loops | fully, as stated policy | 9 |
| Goal-backward planning, TDD, anti-patterns | fully | 9 |
| Research, discussion, decision locking | fully | 9 |
| 67 commands / 107 workflows breadth | condensed to ~8 steps; the long tail (sketch, spike, forensics, UI/security/eval phases, workstreams, graphify) is dropped, recoverable as optional sections | 6 |
| STATE/progress correctness | manual recount, drifts under long sessions | 5 |
| **Blocking** enforcement (PreToolUse deny, commit validation, workflow guard) | not possible — advisory only | 2 |
| Prompt-injection scanning on Read/Write | policy statement only, no scan | 2 |
| Context monitor, statusline, update checks | none | 1 |
| Milestone archiving, locks, worktrees, parallel workstreams | manual, error-prone at scale | 3 |
| Unattended/autonomous runs | not reliable without the guard layer | 2 |

**7/10.** The methodology — which is where the value is — reproduces essentially
whole, because GSD's own methodology is markdown. What is lost is determinism:
markdown can state a rule, only code can refuse. Expect the gap to show up as
drift in state accounting and as skipped gates in long unattended sessions, not
as worse plans.
