...
| every pair on level 4 | `run_all_model_04.bat` |
...
`run_smoke_model_02.bat`), 3 the frontier model at low effort, passed by hand with `--config`, 4 the
...
4. **Write the held-out suite** — roughly half the size of the visible one, same public contract,
   ...
   (chapters 11, 16).
5. **Check the gate conditions hold** for the new project before any result from it is read.
