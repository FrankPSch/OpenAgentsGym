@echo off
REM Full matrix: every methodology on every project, on capability level 3 (.llm_config.e03_claude_opus_5).
...
py -3 "%~dp0run_master.py" --matrix --config .llm_config.e03_claude_opus_5 --workers 1
exit /b %ERRORLEVEL%
